...
rclone_datastore="gd-backup_crypt" # Name of clone remote datastore
############ /END CONFIG
...
if [[ ${COMMAND} == 'rehydrate' ]]; then
    ...
    --drive-chunk-size=32M copy $rclone_datastore:/$rehydrate$CMDARCHIVE $dumpdir \
...
if [[ ${COMMAND} == 'backup-end' ]]; then
    ...
    #ls $rclonedir
    rclone --config /root/.config/rclone/rclone.conf \
    --drive-chunk-size=32M copy $tarfile $rclone_datastore:/$timepath \
    -v --stats=60s --transfers=16 --checkers=16
fi
...
if [[ ${COMMAND} == 'job-end' ||  ${COMMAND} == 'job-abort' ]]; then
    ...
    #ls $rclonedir
    rclone --config /root/.config/rclone/rclone.conf \
    --drive-chunk-size=32M move $_filename4 $rclone_datastore:/$timepath \
    -v --stats=60s --transfers=16 --checkers=16
